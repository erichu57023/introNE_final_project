...
from scipy.integrate import ode, odeint, solve_ivp, LSODA
...
class ODESolver:
    ...
    def integrateUntil(self, target_t, remove_first=False):
        ...
        if self.dt is None:
            ...
            integrator = LSODA(self.dfunc, self.t[-1], self.y[-1], target_t)
            t = []
            y = []
            while (integrator.t < target_t):
                v_step = integrator.y[0]
                if v_step > -40e-5:
                    integrator.y[0] = -80e-5
                    t_rst = integrator.t
                    y_rst = integrator.y
                    del integrator
                    integrator = LSODA(self.dfunc, t_rst, y_rst, target_t)
                integrator.step()
